...
async def start_registration():
    try:
        await send_message(constants.VAUPSHASOVA_LEAGUE_TELEGRAM_ID, fill_template("📝 Погнали регистрироваться на {date}! /add", date=get_next_matchday_formatted()), message_thread_id=constants.TELEGRAM_GATHER_SQUAD_TOPIC_ID)
        log("[Automated message] 📝 Start of registration message sent out.")
    except Exception as e:
        log_error(e)

async def start_waking_up():
    try:
        await send_message(constants.VAUPSHASOVA_LEAGUE_TELEGRAM_ID, "💤 Просыпаемся! /wakeup", message_thread_id=constants.TELEGRAM_GAMEDAY_TOPIC_ID)
...
async def good_morning():
    try:
        ...
        if good_morning_message.any_birthdays_today() == True:
            await send_message(constants.VAUPSHASOVA_LEAGUE_TELEGRAM_ID, str(good_morning_message_text))
...
async def hot_stats():
    try:
        ...
        await send_message(constants.VAUPSHASOVA_LEAGUE_TELEGRAM_ID, str(hot_stats_text))
        log(f"Hot stats sent out.")
    except Exception as e:
        log_error(e)

async def pitch_payment_reminder():
    try:
        ...
        await send_message(constants.VAUPSHASOVA_LEAGUE_TELEGRAM_ID, message_text, message_thread_id=constants.TELEGRAM_ACCOUNTING_TOPIC_ID)
...
async def send_message(chat_id, text, **kwargs):
    await app_instance.bot.send_message(chat_id=chat_id, text=text, **kwargs)
