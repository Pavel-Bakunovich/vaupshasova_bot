            ...
            if validate_CEO_zone(message.from_user.id,get_arguments(message.text)):
                ...
                if matchday is None:
                    if (registered_players_count < 12):
                        ...
                        await check_eligibility_for_adding_to_squad(bot, message, registered_players_count, get_player_name(player), player_id)
                ...
                if (registered_players_count <= 3 and registered_players_count > 0):
                    await bot.reply_to(message, f"⚠️ Внимание, осталось мест: {registered_players_count}")
                else:
                    if registered_players_count == 0:
                        await bot.reply_to(message, "✅ Состав собран, господа присяжные заседатели! Состав собран!")
            else:
...
async def check_eligibility_for_adding_to_squad(bot, message, registered_players_count, name_of_player_to_add, player_id_to_add):
    if registered_players_count < 12:
        ...
        if len(matchday_players_on_chair) > 0:
            ...
            if (player_id != player_id_to_add):
                ...
                await bot.reply_to(message, message_to_player)
